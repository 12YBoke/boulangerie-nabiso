...
import { UserPen, BriefcaseBusiness, House, Smartphone, User, ListOrdered } from "lucide-react"
...
export const UpdateCustomerForm = ({ customer } : Props) => {
  ...
  async function onSubmit(values: z.infer<typeof RegisterCustomersFormFieldsType>) {
    ...
    if(updatecustomer.status === 200) {
      toast({
        title: "Succès",
        description: <Typography variant="body-sm">Les informations du client ont été modifiées avec succès.</Typography>,
      })
      ...
      toast({
        ...
        <Typography variant="body-sm">
          Une erreur est survenue durant la modification des informations du client. Veuillez recommencer l'opération.
        </Typography>,
      })
      router.refresh();
      stopLoading()
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} >
        <Container className="flex flex-col gap-4">
          ...
          <Container className="w-full">
            <Button disabled={update} Icon={UserPen} type="submit" isLoading={isLoading}>Modifier</Button>
          </Container>
        </Container>
      </form>
    </Form>
