  ...
  async function onSubmit(values: z.infer<typeof RegisterFormFieldsType>) {
    ...
    if(password !== confirmpassword) {
      toast({
        ...
        description: <Typography variant="body-sm">Veuillez vous assurer de bien avoir confirmé votre mot de passe</Typography>,
      })
      stopLoading()
    } else {
      ...
      if(registration.status === 200) {
        toast({
          title: "Bienvenue !",
          description: <Typography variant="body-sm">Vos informations ont correctement été enregistré</Typography>,
        })
        ...
        toast({
          ...
          description: <Typography variant="body-sm">Veuillez utiliser une autre adresse email</Typography>,
        })
        stopLoading()
      }
    }
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="relative flex flex-col gap-8 w-[80%] lg:w-[60%]">
        <Container className="flex flex-col gap-4">
          <Typography variant="title-lg" className="hidden md:block">Ajouter un administrateur</Typography>
          <Typography variant="title-base" className="md:hidden">Ajouter un administrateur</Typography>
        </Container>
        ...
        <Container>
          ...
          Vous avez un compte ? <Link href={"/sign-in"} className="text-primary-Default text-title-xs">Connectez vous</Link>
          </Typography>
        </Container>
      </form>
    </Form>
